...
- [x] **D9. Local playback on iOS/Mac** — phone (or Mac) as control *and* listening endpoint. SHIPPED: `LocalPlaybackController` (own AVPlayer + queue + audio session) streams library tracks from the analyzer `/audio` endpoint; `NowPlayingCenter` routes lock-screen/Control-Center/CarPlay commands; `UIBackgroundModes: audio` keeps it alive when backgrounded. Only locally-analysed on-disk tracks play (Qobuz/stream-only are skipped and reported); experimental opt-in Qobuz-CDN fallback in Settings.
  **Promoted to a first-class output (2026-08-10):** the separate `LocalPlayButton` is gone — every play AND queue verb routes through `playToActiveOutput` / `queueToActiveOutput`, so "dit apparaat" behaves like any zone. The local engine gained insert-next/append/remove/reorder (`LocalQueue`, pure + tested) and `QueueView` renders and edits the on-device queue, which Roon's read-only extension API can't do. Remaining: gapless (single `AVPlayer` + manual queue reloads between tracks), offline downloads, native AirPlay route picker.
...
- [x] **F1. Pipeline skeleton + storage.** Migrations v23–v25 (`recommendation_batches`/`_items`, `artist_watchlist`, `discovery_rejections` — additive, no forced resync). Pure stages: `DiscoveryScoring` (digarr's weighted composite: 0.30 consensus/0.25 similarity/0.20 genre/0.15 AI/0.10 feedback + ±0.15 album modifier), `DiscoveryFilter` (in-library/listened/blocked/cooldown/threshold), `DiscoveryPipeline` (merge→resolve→score→filter). New `MusicBrainzDiscoveryClient` (RoonSageCore-local; `AnalyzerCore`'s MB client is genre-only and unreachable from here) — artist resolve + studio release-groups + relationships, reusing the analyzer's rate-limit reservation pattern. `QobuzClient` gained album resolution (`resolveAlbums`/`scoreAlbumCandidate`/`appendAlbumToPlaylist`) — Qobuz is the primary "is-it-playable" resolver (search gives free cover art + release date); MB handles dedup/relationships/gap-fill. 30 unit tests.
  - **Fix (2026-07-01, found via F8's hero-art redesign making it visible):** only `.album`-kind items ever got Qobuz-resolved cover art — `.artist`-kind (the majority of a typical batch) had no art path at all. `QobuzClient.resolveArtistCovers` + pipeline step 3c close the gap: searches by artist name, takes the first hit whose normalised artist matches. Verified live: 24/25 artist-kind items got real art in one run; album-kind resolution itself was already working fine (28/38 matched) — remaining misses are genuine "not on Qobuz" cases the UI already handles ("Niet op Qobuz gevonden"). (analyzer-v1.1.58 · v1.10.75 · ios-v1.7.47)
...
- [~] **F7. Gated producers** (each needs a NEW client + the user's own account — ship one at a time): Discogs **Labels**, Deezer **Flow**, Spotify **Saved Albums** (OAuth — biggest lift, sequence last).
  - [x] **F7a. Discogs Labels (shipped).** New `DiscogsClient` (RoonSageCore-local, mirrors `MusicBrainzDiscoveryClient`'s shape: 1.1s-paced, descriptive User-Agent, retry-once on 429/503, per-run cache). Auth is a single personal access token (`Authorization: Discogs token=…` — verified against live `api.discogs.com` responses + Discogs's own docs, not guessed), not full OAuth1 — appropriate for a single self-hosted user. `DiscogsLabelsProducer`: for up to 6 top-played seed artists, finds ONE release → its primary label (Discogs `entity_type == "1"`, first-listed) → that label's other releases, emitting artists not already known. A genuinely different discovery angle from the Last.fm/MB producers (shared LABEL, not shared listeners or a collaboration credit). Gated via `ProducerContext.discogsToken`; missing token = producer silently skipped, same degrade-gracefully posture as every optional source. Settings → Externe diensten → Discogs (token field, Keychain-backed, same UI pattern as ListenBrainz). No unit tests — network-bound glue with no extractable pure logic, matching every other producer's existing (untested) shape. (v1.10.74 · ios-v1.7.46 · analyzer-v1.1.57)
  - [ ] **F7b. Deezer Flow.** Needs Deezer OAuth (personalized Flow requires user-level auth, not just an API key).
  - [ ] **F7c. Spotify Saved Albums.** OAuth — biggest lift, sequence last (per original plan).
...
- [x] **F10. Ontdek-inzichten (professioneler).** New `GET /discovery/stats` on `LibraryShareServer` → `DiscoveryStatsBuilder` (pure, 7 tests): prune-proof headline (approval rate / saved / skipped) from the persistent watchlist-accept + `discovery_rejections` tables, plus per-producer **source effectiveness** (accept-ratio) and a saved-genre trend over the retained batches' items. Served via `discoveryStatsData`; client fetches over the `isRemote` split (`discoveryStats`). New `DiscoverInsightsView` (List: metric tiles + producer/genre bars), opened as a **sheet** from the feed toolbar — a push would silently fail since the macOS split-detail (`detailView`) has no `NavigationStack`. Shared `DiscoveryProducerLabel` so feed badges + dashboard never drift. Read-only aggregation — no schema change. Source-effectiveness is the feedback loop that later prunes/auto-tunes weak producers (F11). (analyzer-v1.1.53 server · v1.10.70 · ios-v1.7.42)
- [x] **F11. Afstembaar (Fase 3).** `ScoringWeights.tuned(adventurousness:)` (pure, 6 tests): lerps between two hand-tuned "veilig"/"avontuurlijk" anchor presets (each sums to 1.0, so every interpolation does) — veilig trusts consensus/genre-fit, avontuurlijk shifts weight to similarity/AI. New `RoonClient` UserDefaults-backed settings (`discoveryAdventurousness`, `discoveryDisabledProducers`, `discoveryRejectionCooldownDays`) wired into `runDiscoveryPipeline`; `activeDiscoveryProducers` filters by the disabled set but falls back to all producers if that would empty the feed entirely. New analyzer-app screen `DiscoverySettingsView` (dial + per-producer toggles + cooldown stepper) — lives in `RoonSageAnalyzerApp` (like `SonicRadioSettingsView`), **not** the Mac/iOS client, since the pipeline only runs server-side and `/settings` is GET-only (no client→server push exists yet). `DiscoveryProducerLabel` made `public` so feed badges / insights dashboard / tuning settings share one label source. (analyzer-v1.1.54 · v1.10.71 · ios-v1.7.43 — iOS/Mac ship unchanged behaviour, tag together for consistency)
- [x] **F12a. Mood-seeded runs (Fase 4, deel 1).** `mood` parameter on `/discovery/run` — split from F12 since the real CLAP vocabulary turned out to be 6 labels (`danceable/aggressive/happy/party/relaxed/sad` — `clap_mel.json`'s `mood_labels`), not the defensive 22-entry Dutch translation table. Candidates don't own an embedding (not-yet-owned = never analyzed), so mood can't bias the Score stage directly; instead `MoodSeeding.topArtists` (pure, 8 tests) ranks OWNED, analyzed artists by mean mood score (reusing the exact per-track scores that already power mood-radio buckets) and that list REPLACES the top-played seed every producer consumes. `AIPicksProducer` additionally reads `ProducerContext.mood` and leans its prompt toward the vibe. Empty-library-presence-for-this-mood falls back to the unbiased seed (AI producer still leans in) — never a hard failure. A mood run always bypasses the skip-if-unchanged guard (deliberate one-off, never a redundant repeat) and is stored with `trigger: "mood:<key>"` (existing TEXT column, no migration). New toolbar menu in `DiscoverFeedView` (theatermasks icon) next to Ververs. `RoonClient.knownMoodKeys` + widened-to-public `moodLabel(_:)` are the single source for both the radio-bucket labels and this picker. (analyzer-v1.1.55 · v1.10.72 · ios-v1.7.44)
- [x] **F12b. Wekelijkse digest (Fase 4, deel 2).** Correction to F12's original plan: there is NO push infrastructure anywhere in native (grepped clean — no `UNUserNotificationCenter`, no APNs, no webhook notifier; the only "Live Activity" is Now Playing/ActivityKit, unrelated). Went with a **local, foreground-triggered** notification instead of building APNs from scratch (self-hosted, no Apple push credentials in this repo). `DigestSelection` (pure, 9 tests): dedupes still-pending album recommendations by `dedup_key` across every RETAINED batch (not just the newest) and ranks by score; `weekKey(for:)` uses `.yearForWeekOfYear` (not `.year`) for correct ISO week-boundary handling, verified against Python's `isocalendar()`. Batch retention bumped 3→14 (`discoveryBatchRetention`) so a full week of daily runs survives pruning. `RoonClient.buildWeeklyDigestIfDue` (hourly-checked via `startDigestSchedule`, analyzer-only): on the configured weekday (`discoveryDigestWeekday`, default Monday, picker in `DiscoverySettingsView`) and only once per ISO week (watermark in `discoveryLastDigest`), saves the top 20 into a dated Qobuz playlist ("Ontdekkingen — 2026-W27") via the existing find-or-create `appendAlbumToPlaylist`. Any skip path (no Qobuz configured, all saves failed) leaves the watermark untouched so the next hourly check retries. New `GET /discovery/digest-status`; new `RoonSageUI.DiscoveryDigestNotifier` — each client independently compares the server's digest week against its OWN locally-acknowledged week (UserDefaults, per-device) and fires a local `UNUserNotificationCenter` alert, so a Mac + iPhone both notify without a shared "seen" flag. Wired into both app targets' existing foreground hooks (`RoonSageApp`'s launch `.task`, `RoonSageiOSApp`'s `scenePhase == .active`) — no new lifecycle plumbing. Verified: SPM `swift build`/`-c release` clean, iOS target `xcodebuild … -destination "generic/platform=iOS Simulator"` BUILD SUCCEEDED, full suite 300/300. (analyzer-v1.1.56 · v1.10.73 · ios-v1.7.45)
